...
const ACCOUNT_FIELDS = [
  ...
  "monday_board_id", "zendesk_org_id", "whatconverts_profile_id", "quickbooks_customer_id", "locations", "logo_url",
];
...
Deno.serve(async (req) => {
  try {
    ...
    if (action === "analytics") {
      ...
      // analytics entirely: anyone flagged is_staff, plus anyone whose email
      // is on an Alloy domain (alloygp.co et al.). Combined with dropping
      // events whose account doesn't resolve, this clears the legacy "Unknown"
      // row (old staff / null-account activity recorded before tracking
      // started skipping staff).
